<?php
/**
 * API du marché obligataire, extrait du tableau des obligations des
 * Bulletins Officiels de la Cote (BOC)
 * Endpoint: api_bulletin_bond_metrics.php
 *
 * Mirror de api_bulletin_stock_metrics.php, mais pour les lignes
 * obligataires (souveraines, entreprises, GSS, FCTC, Sukuk, convertibles)
 * plutôt que les actions — délègue à class/BulletinBondMetricsService.php.
 */
...
require_once 'class/BulletinBondMetricsService.php';

class BulletinBondMetricsAPI {
    private $crud;
    ...
    /**
     * Déclenche (ou réutilise le cache pour) l'extraction du marché
     * obligataire d'un bulletin.
     */
    private function extract($input) {
        ...
        $service = new BulletinBondMetricsService($this->crud);
    ...
    /**
     * Vue d'ensemble de toutes les lignes obligataires déjà extraites,
     * filtrable par symbole/catégorie/période. Inclut aussi la liste des
     * bulletins dont le texte est disponible mais qui n'ont pas encore été
     * extraits.
     */
    ...
    /**
     * Symboles obligataires distincts déjà rencontrés, pour peupler un
     * sélecteur « choisir un titre » côté frontend.
     */
    private function symbols() {
        $service = new BulletinBondMetricsService($this->crud);
        return ['success' => true, 'data' => ['symbols' => $service->listSymbols()]];
    }
}

// Exécution
$api = new BulletinBondMetricsAPI();
